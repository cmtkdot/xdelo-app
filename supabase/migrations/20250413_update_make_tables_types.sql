...
-- Update database types for make-related tables and fix issues with JSONB types

-- First, add missing types and interfaces for Make extensions
CREATE EXTENSION IF NOT EXISTS pgcrypto;

-- Fix upsert_media_message function to ensure proper JSONB types
...
BEGIN
  ...
  IF v_message_id IS NOT NULL THEN
    ...
    IF v_caption_changed AND v_existing_analyzed_content IS NOT NULL AND p_old_analyzed_content IS NULL THEN
      ...
      UPDATE public.messages SET
        ...
        -- For old_analyzed_content we need a special approach to handle the jsonb vs jsonb[] type issue
        old_analyzed_content = 
          CASE 
            WHEN p_old_analyzed_content IS NOT NULL THEN 
              -- Use the provided value
              p_old_analyzed_content
            WHEN old_analyzed_content IS NOT NULL THEN
              -- Append existing analyzed_content to old_analyzed_content as jsonb
              jsonb_build_array(v_existing_analyzed_content)
            ELSE
              -- Create new array
              jsonb_build_array()
          END,
    ...
    ELSE
      ...
      UPDATE public.messages SET
        ...
        caption_data = p_caption_data,
        analyzed_content = p_analyzed_content,
        -- Only update old_analyzed_content if explicitly provided
  ...
  ELSE
    ...
    INSERT INTO public.messages (
      ...
      CASE WHEN p_old_analyzed_content IS NOT NULL THEN p_old_analyzed_content ELSE jsonb_build_array() END,
      p_correlation_id,
      '[]'::jsonb,
      p_message_data
    )
...
VALUES (
    '13.0',
    'update-make-tables-types',
    NOW(),
    'Updated upsert_media_message to correctly handle JSONB vs JSONB[] type issues'
);
