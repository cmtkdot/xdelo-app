...
-- Apply indexes and schema changes to improve performance
...
-- These indexes will significantly speed up duplicate checks and message retrieval
DO $$
BEGIN
...
-- 3. Function to reset or kill long-running queries causing locks/timeouts
CREATE OR REPLACE FUNCTION public.xdelo_kill_long_queries(
    timeout_threshold_seconds INTEGER DEFAULT 10
) RETURNS TABLE (
    pid INTEGER,
...
BEGIN
    ...
    WHERE 
        -- Only kill queries in specific states
...
COMMENT ON FUNCTION public.xdelo_kill_long_queries IS 'Identifies and terminates long-running queries exceeding the specified timeout threshold';

-- 4. Create a procedure to migrate data from telegram_data to telegram_metadata
-- This helps gradually transition away from the large telegram_data field
...
COMMENT ON PROCEDURE public.xdelo_migrate_telegram_data IS 'Migrates data from the large telegram_data field to the more efficient telegram_metadata field'; 