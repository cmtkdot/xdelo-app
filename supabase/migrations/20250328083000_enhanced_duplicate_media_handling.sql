-- Migration: Enhanced Duplicate Media Handling
-- Updates md_handle_duplicate_media_message and handle_media_message
-- to conditionally process duplicates based on caption changes or missing analysis.

-- Update the md_handle_duplicate_media_message function to return jsonb with status info
...
RETURNS jsonb -- Change return type to jsonb for more information
LANGUAGE plpgsql
SECURITY DEFINER
...
COMMENT ON FUNCTION public.md_handle_duplicate_media_message(text, bigint, bigint, jsonb)
IS 'Handles incoming media messages, checking for exact duplicates or duplicates by file_unique_id. Updates existing records for duplicates, conditionally resetting processing state if caption changed or analysis is missing. Creates new records otherwise. Returns jsonb status.';


-- Update the handle_media_message RPC function to return the expanded result
...
RETURNS jsonb -- Change to return more information
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public -- Ensure correct schema context
AS $$
...
COMMENT ON FUNCTION public.handle_media_message(bigint, bigint, text, jsonb)
IS 'RPC endpoint to handle incoming media messages. Calls md_handle_duplicate_media_message to manage DB records and returns jsonb status indicating if further processing is needed.';
