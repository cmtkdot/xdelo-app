...
-- Fix the p_caption_data parameter type from TEXT to JSONB in upsert_media_message function
-- This migration corrects a type inconsistency where p_caption_data was defined as TEXT
-- but used as JSONB in various places within the function

-- Drop existing function to completely replace it
DROP FUNCTION IF EXISTS public.upsert_media_message;

-- Recreate the function with the proper p_caption_data parameter type
CREATE OR REPLACE FUNCTION public.upsert_media_message(
    ...
    p_caption_data jsonb DEFAULT NULL::jsonb, -- Changed from TEXT to JSONB
...
BEGIN
  ...
  IF v_message_id IS NOT NULL THEN
    ...
    IF v_caption_changed AND v_existing_analyzed_content IS NOT NULL AND p_old_analyzed_content IS NULL THEN
      ...
      UPDATE public.messages SET
        ...
        caption_data = p_caption_data, -- Now properly treated as JSONB
  ...
  ELSE
    ...
    INSERT INTO public.messages (
      ...
      p_forward_info,
      p_media_group_id,
      p_caption_data, -- Now properly treated as JSONB
...
VALUES (
    '11.1',
    'fix-caption-data-type',
    NOW(),
    'Changed p_caption_data parameter type from TEXT to JSONB in upsert_media_message function'
);
