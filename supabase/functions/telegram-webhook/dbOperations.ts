...
/**
 * Logs a processing event for tracking and debugging
 * Extracted from _shared/databaseOperations.ts to consolidate in telegram-webhook
 */
export async function xdelo_logProcessingEvent(
  ...
  try {
    // Only initialize supabase client when needed
    const supabaseUrl = Deno.env.get('SUPABASE_URL');
    const supabaseKey = Deno.env.get('SUPABASE_SERVICE_ROLE_KEY');
    
    if (!supabaseUrl || !supabaseKey) {
      console.error('Missing Supabase credentials for logging event');
      return;
    }
    ...
    if (error) {
      ...
      // Try again with simplified metadata if it might be too large
      try {
        if (Object.keys(metadata).length > 0) {
          const { error: retryError } = await supabase
            ...
            .insert({
              ...
              correlation_id: correlationId,
              metadata: {
                original_metadata_keys: Object.keys(metadata),
                correlation_id: correlationId,
                event_timestamp: new Date().toISOString(),
                log_retry: true
              },
              error_message: `${errorMessage || ''} (Simplified log due to: ${error.message})`,
              created_at: new Date().toISOString()
            });
            
          if (retryError) {
            console.error('Failed retry logging processing event:', retryError);
          }
        }
      } catch (retryErr) {
    ...
    console.error('Exception in xdelo_logProcessingEvent:', 
      error instanceof Error ? error.message : String(error));
  }
}