...
export function xdelo_isViewableMimeType(mimeType) {
  ...
  const tempProcessor = new MediaProcessor({}, "", "");
...
export function xdelo_getExtensionFromMimeType(mimeType) {
...
export function xdelo_detectMimeType(telegramData) {
...
export function xdelo_generateStoragePath(fileUniqueId, mimeType) {
...
export function xdelo_validateAndFixStoragePath(fileUniqueId, mimeType) {
...
export function xdelo_getUploadOptions(mimeType) {
 ...
 */ async function xdelo_fetchWithRetry(url, options = {}, maxRetries = 5, initialRetryDelay = 500) {
  console.warn("xdelo_fetchWithRetry is deprecated. Use MediaProcessor.fetchWithRetry instead.");
  // Create a temporary MediaProcessor instance to use the method
  const tempProcessor = new MediaProcessor({}, Deno.env.get("TELEGRAM_BOT_TOKEN") || "", "");
...
export async function xdelo_findExistingFile(supabase, fileUniqueId) {
...
export async function xdelo_verifyFileExists(supabase, storagePath, bucket = "telegram-media") {
...
export async function xdelo_uploadMediaToStorage(storagePath, fileData, mimeType, messageId, bucket = "telegram-media") {
  console.warn("xdelo_uploadMediaToStorage is deprecated. Use MediaProcessor.uploadMediaToStorage instead.");
  // Create a temporary MediaProcessor instance to use the method
  const tempProcessor = new MediaProcessor({}, "", "");
  return tempProcessor.uploadMediaToStorage(storagePath, fileData, mimeType, messageId, bucket);
}
/**
 * @deprecated Use MediaProcessor.downloadMediaFromTelegram instead.
 * This function will be removed in a future release.
 */ // Download media from Telegram with improved error handling
export async function xdelo_downloadMediaFromTelegram(fileId, fileUniqueId, mimeType, telegramBotToken) {
  console.warn("xdelo_downloadMediaFromTelegram is deprecated. Use MediaProcessor.downloadMediaFromTelegram instead.");
  // Create a temporary MediaProcessor instance to use the method
  const tempProcessor = new MediaProcessor({}, telegramBotToken, "");
  try {
...
export async function xdelo_processMessageMedia(telegramData, fileId, fileUniqueId, telegramBotToken, messageId) {
  ...
  return Promise.resolve({
    ...
    isDuplicate: false,
    fileInfo: {},
    error: "Deprecated function cannot be adapted easily."
  });
 ...
 * );
 */ export function createMediaProcessor(supabaseClient, telegramBotToken, storageBucket = "telegram-media") {
