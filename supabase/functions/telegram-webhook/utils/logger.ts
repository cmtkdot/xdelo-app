...
import type { SupabaseClient as _SupabaseClient } from "https://esm.sh/@supabase/supabase-js@2.38.4";
...
export async function logProcessingEvent(
  ...
  if (error) {
    console.error(`${logPrefix} ${message}`, { ...metadata, error: error instanceof Error ? error.message : String(error) });
  } else {
