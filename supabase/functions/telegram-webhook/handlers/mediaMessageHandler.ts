...
async function xdelo_handleNewMediaMessage(
  ...
  try {
    ...
    const isDuplicate = await checkDuplicateFile(supabaseClient, undefined, undefined, mediaContent.file_unique_id);

    if (isDuplicate) {
      context.logger?.info(`Found duplicate media with file_unique_id ${mediaContent.file_unique_id}, skipping processing`);
    ...
    // Create the message with a properly typed logger
    const result = await createMessage(
      supabaseClient, 
      messageInput, 
      // Use a minimal logger adapter if logger is undefined
      context.logger || {
