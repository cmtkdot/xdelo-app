...
export async function manualParse(caption: string): Promise<ParsedContent> {
  ...
  const confidence = calculateConfidence(result, fallbacks_used);
...
function calculateConfidence(result: ParsedContent, fallbacks: string[]): number {
  ...
  if (!result.product_name) confidence -= 0.3;
  if (!result.product_code) confidence -= 0.1;
  if (!result.vendor_uid) confidence -= 0.1;
  if (!result.purchase_date) confidence -= 0.1;
  if (!result.quantity) confidence -= 0.1;
  
  // Reduce confidence for each fallback used
  confidence -= (fallbacks.length * 0.1);
