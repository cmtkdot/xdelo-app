...
export async function analyzeCaption(caption: string): Promise<ParsedContent> {
  try {
    ...
    const cleanResult: ParsedContent = {
      product_name: result.product_name?.trim() || caption.split('#')[0]?.trim() || 'Untitled Product',
      product_code: result.product_code,
      vendor_uid: result.vendor_uid?.toUpperCase(),
      purchase_date: result.purchase_date,
      quantity: result.quantity ? Math.floor(Number(result.quantity)) : undefined,
    ...
    // Validate quantity
    if (cleanResult.quantity !== undefined && (isNaN(cleanResult.quantity) || cleanResult.quantity <= 0)) {
      delete cleanResult.quantity;
    }
