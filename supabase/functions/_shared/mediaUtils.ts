...
export function xdelo_detectMimeType(telegramData: any): string {
  ...
  console.log(
    "Detecting MIME type from telegram data:",
    JSON.stringify({
  ...
  console.warn(
    "Could not detect MIME type from telegram data, falling back to octet-stream"
  );
...
export function xdelo_generateStoragePath(
  fileUniqueId: string,
  mimeType: string
): string {
  if (!fileUniqueId) {
  ...
  const extension = xdelo_getExtensionFromMimeType(
...
export function xdelo_validateAndFixStoragePath(
  fileUniqueId: string,
  mimeType: string
): string {
  if (!fileUniqueId) {
  ...
  return xdelo_generateStoragePath(
    fileUniqueId,
    mimeType || "application/octet-stream"
  );
}
...
async function xdelo_fetchWithRetry(
  ...
  while (retryCount < maxRetries) {
    try {
      ...
      console.log(
        `Attempt ${retryCount + 1}/${maxRetries} to fetch ${url.substring(
          0,
          100
        )}...`
      );
      ...
      console.log(
        `Successfully fetched ${url.substring(0, 100)} on attempt ${
          retryCount + 1
        }`
      );
      return response;
      ...
      const isServerError =
        error.message.includes("5") && error.message.includes("HTTP error");

      // Enhanced logging with error classification
      console.warn(`Fetch attempt ${retryCount}/${maxRetries} failed for ${url.substring(
        0,
        100
      )}:
        Error: ${error.message}
        ...
        Will ${
          retryCount < maxRetries ? `retry in ${retryDelay}ms` : "not retry"
        }`);

      // If we've reached max retries, throw the last error
      if (retryCount >= maxRetries) {
        console.error(
          `All ${maxRetries} retry attempts failed for ${url.substring(0, 100)}`
        );
        throw new Error(
...
export async function xdelo_uploadMediaToStorage(
  ...
  try {
    console.log(
      `Uploading file to storage: ${storagePath} with MIME type: ${mimeType}`
    );
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        ...
        uploadError = error;
        console.warn(
          `Upload attempt ${attempt}/3 failed for ${storagePath}: ${error.message}`
        );
        ...
        uploadError = err;
        console.warn(
          `Upload attempt ${attempt}/3 failed for ${storagePath}: ${err.message}`
        );
    ...
    if (uploadError) {
      throw new Error(
        `Storage upload failed after 3 attempts: ${uploadError.message}`
      );
    }
...
export async function xdelo_downloadMediaFromTelegram(
  ...
  try {
    ...
    console.log(
      `Starting download process for file ${fileId} (${fileUniqueId})`
    );
    ...
    if (!fileInfo.result?.file_path) {
      throw new Error(
        `Invalid file info response from Telegram: ${JSON.stringify(fileInfo)}`
      );
    }

    console.log(
      `Successfully retrieved file path: ${fileInfo.result.file_path}`
    );
    ...
    if (!detectedMimeType || detectedMimeType === "application/octet-stream") {
      ...
      const extension = fileInfo.result.file_path
        .split(".")
        .pop()
        ?.toLowerCase();
      if (extension) {
        ...
        if (extensionMimeMap[extension]) {
          ...
          console.log(
            `Detected MIME type from file extension: ${detectedMimeType}`
          );
        }
      }
    }
    ...
    console.log(
      `Generated storage path: ${storagePath} with MIME type: ${detectedMimeType}`
    );
...
export async function xdelo_processMessageMedia(
  ...
  try {
    ...
    if (exists && existingMessage) {
      console.log(
        `Duplicate file detected: ${fileUniqueId}, reusing existing file`
      );
    ...
    console.log(
      `New file detected: ${fileUniqueId}, downloading from Telegram`
    );
    ...
    if (!downloadResult.success || !downloadResult.blob) {
      throw new Error(
        downloadResult.error || "Failed to download media from Telegram"
      );
    }
    ...
    if (!uploadResult.success) {
      throw new Error(
        uploadResult.error || "Failed to upload media to storage"
      );
    }
    ...
    const fileInfo = {
      ...
      mime_type_original:
        telegramData.video?.mime_type || telegramData.document?.mime_type,
      file_id: fileId,
      file_unique_id: fileUniqueId,
      width: media?.width,
      height: media?.height,
