...
export class Logger {
  correlationId;
  source;
  constructor(correlationId: string, source = 'telegram-webhook'){
   ...
   */
  retryAttempt(operationName: string, attemptNumber: number, maxRetries: number, error: Error | string | undefined, metadata = {}) {
   ...
   * @param metadata - Additional metadata
   */
  retrySuccess(operationName: string, attemptCount: number, totalTimeMs: number, metadata = {}) {
   ...
   * @param metadata - Additional metadata
   */
  retryExhausted(operationName: string, attemptCount: number, totalTimeMs: number, finalError: Error | string, metadata = {}) {
   ...
   */
  async logToDatabase(level: string, message: string, metadata = {}) {
    try {
 ...
 */
export function createRetryLogger(correlationId: string) {
 ...
 * @param metadata - Additional metadata to include
 */
export function logWithCorrelation(correlationId: string, message: string, level = 'info', source = 'telegram-webhook', metadata = {}) {
