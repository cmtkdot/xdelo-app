...
// Helper function to determine if a message is a video
const isVideoMessage = (message: Message): boolean => {
  return message.mime_type?.startsWith("video/") || false;
};
...
export function useGalleryFilters({
  ...
  const filteredMessages = useMemo(() => {
    ...
    console.log(
      `Filter result: ${result.length} messages after applying '${filter}' filter`
    );
    return result;
  }, [messages, filter]);
