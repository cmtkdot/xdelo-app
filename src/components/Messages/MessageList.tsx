  ...
  return <div className="space-y-6">
      <div className="bg-white p-4 rounded-lg shadow mb-4">
        <div className="flex justify-between items-center mb-4">
          ...
          <Button onClick={onProcessAll} disabled={processAllLoading || !pendingCount && !errorCount} className="flex items-center">
      ...
      <div className="space-y-4">
        {messages.map(message => <Card key={message.id} className="overflow-hidden">
            <CardContent className="p-0">
              <div className="p-4">
                <div className="flex justify-between items-start">
                  <div className="space-y-1">
                    ...
                    {message.analyzed_content?.product_name && <p className="text-sm">
                        <span className="font-semibold">Product:</span> {message.analyzed_content.product_name}
                      </p>}
                    {message.analyzed_content?.vendor_uid && <p className="text-sm">
                        <span className="font-semibold">Vendor:</span> {message.analyzed_content.vendor_uid}
                      </p>}
                    {message.analyzed_content?.purchase_date && <p className="text-sm">
                  ...
                  <div className="flex items-center">
                    ...
                    {(message.processing_state === 'error' || message.processing_state === 'pending') && <Button size="sm" variant="outline" className="ml-2" onClick={() => {
              ...
              {message.public_url && <div className="h-48 bg-gray-100">
                  {message.mime_type?.startsWith('image/') ? <img src={message.public_url} alt="Message media" className="h-full w-full object-contain" /> : message.mime_type?.startsWith('video/') ? <video src={message.public_url} controls className="h-full w-full object-contain" /> : <div className="flex items-center justify-center h-full">
                      <p className="text-gray-500">Media not supported</p>
                    </div>}
                </div>}
              {message.error_message && <div className="p-3 bg-red-50 text-red-800 text-sm">
