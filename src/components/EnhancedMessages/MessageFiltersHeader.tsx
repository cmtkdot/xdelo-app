...
export function MessageFiltersHeader({
  ...
  return <div className="space-y-2">
      ...
      {isFiltersPanelOpen && <div className="w-full border rounded-lg p-3 bg-card shadow-sm">
          <EnhancedFiltersPanel />
        </div>}
      
      {/* Show/hide filters button when panel is closed */}
      {!isFiltersPanelOpen && onToggleFiltersPanel && <div className="flex justify-start">
          
        </div>}
      
      {/* Hide filters button when panel is open */}
      {isFiltersPanelOpen && onToggleFiltersPanel && <div className="flex justify-end">
          <Button variant="ghost" size="sm" onClick={onToggleFiltersPanel} className="gap-2">
