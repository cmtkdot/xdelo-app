...
export function MediaDisplay({ 
  ...
  // Set type based on mime type if not explicitly provided
  const isVideo = type === 'video' || mimeType?.startsWith('video/') || /\.(mp4|mov|webm|avi)$/i.test(url);
  ...
  const handleLoadError = (mediaType: 'image' | 'video') => {
    console.error(`Failed to load ${mediaType}`, { url, mimeType });
    setIsLoading(false);
  ...
  const VideoPlayer = () => (
    <AspectRatio ratio={aspectRatio} className="w-full max-w-full h-auto">
      <video
        src={url}
