...
export class RealtimeChat {
  ...
  async connect() {
    try {
      ...
      this.ws = new WebSocket(`wss://xjhhehxcxkiumnwbirel.supabase.co/functions/v1/realtime-chat`);
      ...
      this.ws.onmessage = async (event) => {
        ...
        if (data.type === 'response.audio.delta') {
          ...
          const audioData = this.decodeBase64Audio(data.delta);
  ...
  private decodeBase64Audio(base64: string): Uint8Array {
